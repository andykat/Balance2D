...
public class main : MonoBehaviour {
	public GameObject topCylPreFab;
	public GameObject botCylPreFab;
	public GameObject groundRectPreFab;
	private static GameObject[] topCyl = new GameObject[100];
	private static GameObject[] botCyl = new GameObject[100];
	private static GameObject[] groundRect = new GameObject[100];
	public static int inputN = 1;
	...
	void Update () {
		...
		for (int i=0; i<networks.Count; i++) {
			...
			if (topCyl[i].transform.position.y - botCyl[i].transform.position.y < 0.7f) {
				networks[i].fitness = timeCurrent;
				resetEverything (i);
				finished++;
				finishedFlag[i] = true;
			}
			topCyl[i].transform.position = new Vector3 (topCyl[i].transform.position.x, topCyl[i].transform.position.y, 0.0f);
			topCyl[i].transform.eulerAngles = new Vector3 (90.0f, 0.0f, 0.0f);
			botCyl[i].transform.position = new Vector3 (botCyl[i].transform.position.x, botCyl[i].transform.position.y, 0.0f);
			botCyl[i].transform.eulerAngles = new Vector3 (90.0f, 0.0f, 0.0f);
		}
		if (finished == networks.Count) {
			loop();
			finished = 0;
			for (int i=0; i<networks.Count; i++) {
				initEverything (i);
	...
	public void initEverything(int index){
		int x = index / 10;
		int y = index % 10;
		float sx = ((float)x) * 6.5f;
		float sy = ((float)y) * 3.0f;


		topCyl[index] = Instantiate (topCylPreFab) as GameObject;
		botCyl[index] = Instantiate (botCylPreFab) as GameObject;
		groundRect [index] = Instantiate (groundRectPreFab) as GameObject;
		
		topCyl[index].transform.position = new Vector3 (sx + 0.02f, 0.7f + sy, 0.0f);
		botCyl [index].transform.position = new Vector3 (sx, sy - 0.45f , 0.0f);
		groundRect [index].transform.position = new Vector3 (sx, sy - 1.0f, 0.0f);
	}
	public void resetEverything(int index){
		Destroy (botCyl [index]);
		Destroy (topCyl [index]);
		Destroy (groundRect [index]);
	}
	...
	public static void loop(){
		...
		/*for (int i=0; i<networks.Count; i++) {
			networks[i].fitness = balanceFitness(i);
		}*/
	...
	private static double balanceFitness(int index){
		return topCyl [index].transform.position.y;
	}
	...
	private static void lukeUseTheForce(int index){
		...
		inputs.Add ((double)(topCyl [index].transform.position.x - botCyl [index].transform.position.x));
